...
bool app_pos_reserve(struct app_pos_position *p, keel_arena *a, size_t n) {
    p->x = (f32 *)keel_arena_alloc2(a, sizeof(f32), _Alignof(f32), n);
    p->y = (f32 *)keel_arena_alloc2(a, sizeof(f32), _Alignof(f32), n);
    p->len = 0;
    p->cap = (p->x && p->y) ? n : 0;
    return p->cap > 0;
}
...
bool app_pos_grid_init(struct app_pos_grid *g, keel_arena *a, size_t rcap, size_t ccap) {
    g->v = (i32 *)keel_arena_alloc2(a, sizeof(i32), _Alignof(i32), rcap * ccap);
    g->rows = 0;
    g->cols = 0;
    g->rcap = g->v ? rcap : 0;
    g->ccap = g->v ? ccap : 0;
