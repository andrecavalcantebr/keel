...
# cycles.
structure=0
for h in $(find c23 c11 cases -name '*.h' ! -name '*.type.h' | sort); do
  [ -f "${h%.h}.type.h" ] || { printf 'STRUCT %s — missing %s\n' "$h" "${h%.h}.type.h"; structure=$((structure+1)); }
done
for d in $(find c23 c11 cases -name '*.proto.h' | sort); do
...
for t in $(find c23 c11 cases -name '*.type.h' | sort); do
