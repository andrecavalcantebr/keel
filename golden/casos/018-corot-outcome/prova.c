...
int main(void) {
    ...
    assert(resultado.code == keel_outcome_NONE && resultado.value == 42);
    keel_outcome_i32 objetos[2] = {{-1, 0}, {-2, 0}};
    size_t indice = 0;
    i32 valor = 80;
    copia = keel_outcome_i32_win1(&objetos[indice++], valor++);
    assert(indice == 1 && valor == 81);
    assert(objetos[0].code == 0 && objetos[0].value == 80);
    assert(copia.code == 0 && copia.value == 80 && objetos[1].code == -2);
