/* prova do caso 021: a cauda `else` sobre atribuição, com os resultados
   declarados fora do laço. NÃO é saída do transpilador. */
...
int main(void) {
    ...
    if (k != 3) return 1;                                   /* o quarto nome falta: `else break` */

    if (!eq(app_reg_people[0].name, "ana", 3)) return 2;
    if (!eq(app_reg_people[0].address, "rua um", 6)) return 3;
    if (app_reg_people[0].birth != 1.5) return 4;

    if (!eq(app_reg_people[1].address, "", 0)) return 5;    /* default da atribuição */
