...
int app_align_main(int argc, char **argv) {
    ...
    for (size_t d = 0; d < 8; d++) {
        ...
        i32  *p = (i32 *)keel_arena_alloc(&a, 3, sizeof(i32), alignof(i32));
        f64  *q = (f64 *)keel_arena_alloc(&a, 2, sizeof(f64), alignof(f64));
        app_align_Vec8 *v = (app_align_Vec8 *)keel_arena_alloc(&a, 1, sizeof(app_align_Vec8), alignof(app_align_Vec8));
        if (!p || !q || !v) return 2;
